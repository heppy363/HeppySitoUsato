...
* package `app/database/` con configurazione SQLAlchemy 2.x asincrona, engine condiviso, factory `async_sessionmaker`, context manager di sessione con rollback degli errori e chiusura deterministica;
...
```bash
poetry check
poetry run pytest tests/test_app.py tests/test_network.py tests/test_providers.py tests/test_ebay_provider.py tests/test_aggregation.py tests/test_health.py tests/test_search.py tests/test_rate_limit.py tests/test_cache.py tests/test_database.py -q
...
Il controllo database di `GET /health` usa l'engine condiviso e resta degradato finche il servizio PostgreSQL non e raggiungibile. La verifica live contro PostgreSQL e la configurazione Alembic non fanno parte di questo incremento.
