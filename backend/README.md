...
* package `app/services/` con il primo contratto dell'Aggregation Engine sopra `ProviderRegistry`, basato su `AggregationRequest`, `AggregationResponse`, `RegistryAggregationService` e `RankingService`, con esecuzione parallela iniziale via `asyncio.gather`, deduplicazione per `(platform, external_id)`, primo merge conservativo, ranking euristico iniziale, primi filtri prezzo e ordinamento finale deterministico dei risultati aggregati;
...
```bash
poetry check
poetry run pytest tests/test_app.py tests/test_network.py tests/test_providers.py tests/test_ebay_provider.py tests/test_aggregation.py -q
