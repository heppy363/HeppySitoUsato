...
* package `app/services/` con il primo contratto dell'Aggregation Engine sopra `ProviderRegistry`, basato su `AggregationRequest`, `AggregationResponse` e `RegistryAggregationService` con esecuzione parallela iniziale via `asyncio.gather`, deduplicazione per `(platform, external_id)` e primo merge conservativo dei risultati;
