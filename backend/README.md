...
* package `app/services/` con il primo contratto dell'Aggregation Engine sopra `ProviderRegistry`, basato su `AggregationRequest`, `AggregationResponse`, `RegistryAggregationService` e `RankingService`, con esecuzione parallela iniziale via `asyncio.gather`, deduplicazione per `(platform, external_id)`, primo merge conservativo e ranking euristico iniziale dei risultati;
