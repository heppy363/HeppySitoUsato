...
* endpoint `GET /search` con validazione della query, paginazione, selezione piattaforme, filtri prezzo, primo parametro `sort` configurabile (`relevance`, `price_asc`), risposta aggregata tipizzata, errore `400` per piattaforme sconosciute e schema OpenAPI generato automaticamente;
